...
- **Person**: `name`, `aliases`, `phone_number`
  - **Validation:** You are STRICTLY FORBIDDEN from adding a `content` or `title` property to a `Person` node. A `Person` node is a pure identity anchor, not a narrative block. If you update a Person, only update `name` or `aliases`.
...
- **Task**: `title` (REQUIRED for CREATE_NODE), `content` (REQUIRED for CREATE_NODE), `status` (planned|active|completed|abandoned|stale), `due_date`, `priority`, `aliases`, `verbatim` (Exact source text, MUST include the speaker label, e.g. '[Name]: quote'), `needs_clarification`, `clarification_basis`, `group_mentions` (Array of objects like `[{"speaker": "name", "phrase": "my team", "quote": "...", "note": "..."}]` to record casual organizational references without creating Circle nodes).
  - **Outgoing Edges (Allowed):**
...
- **Event**: `title` (REQUIRED for CREATE_NODE), `content` (REQUIRED for CREATE_NODE), `status` (planned|active|completed|cancelled|stale), `event_date`, `aliases`, `verbatim` (Exact quote referencing the event, MUST include the speaker label), `needs_clarification`, `clarification_basis`, `group_mentions` (Array of objects like `[{"speaker": "name", "phrase": "panitia", "quote": "...", "note": "..."}]` to record casual organizational references).
...
- **Insight**: `title` (REQUIRED for CREATE_NODE), `content` (REQUIRED for CREATE_NODE), `category` (personality|relationship_dynamic|preference|pattern), `confidence` (high|medium|low), `aliases`, `verbatim` (Exact statement that triggered this insight, MUST include the speaker label), `needs_clarification`, `clarification_basis`

- **Circle**: `title` (REQUIRED for CREATE_NODE), `content` (REQUIRED for CREATE_NODE. Describe the group's purpose or context), `aliases` (e.g. acronyms or casual community names), `verbatim` (Exact quote referencing the group, MUST include the speaker label), `needs_clarification`, `clarification_basis`
  - **Note:** Circle creation via this pipeline is currently disabled — see `skill_commit.md` Rule 15. This section describes the node shape for the downstream batch promotion job.
  - **Validation:** You are STRICTLY FORBIDDEN from creating a `Circle` node without linking at least one `Person` to it via a `MEMBER_OF` edge. A floating Circle node with no members is a critical graph error. If you create a Circle, you MUST update the relevant Person nodes and add `MEMBER_OF` edges pointing to the new Circle.
...
- **Title Property (The Search Index):** You MUST generate a highly condensed 3-5 word `title` for the node (e.g. "Reimburse Rapat BPH Gacoan"). During an `UPDATE_NODE`, if the node's purpose is clarified, you MUST overwrite the `title` to reflect the newly discovered keywords. The `title` acts as the dynamic search index, protecting the historical `content` from destructive formatting edits.
...
**Evidence Refs:** Every edge MUST include `evidence_refs`. You must quote the exact transcript line or vault data that proves the relationship. If you cannot quote direct evidence, do not add the edge. If your strongest evidence for a commitment is a short reply (e.g. short affirmative chat replies), you MUST include a second `evidence_ref` quoting the question or directive it responds to, so the short reply has context.

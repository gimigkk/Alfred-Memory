...
1. **Conversational Commitment (Edges):** An `ASSIGNED_TO` relationship exists if and only if a participant accepts a responsibility, is given a directive without objection, or confirms their role regarding a task.
   ...
   **MANDATORY SYSTEM CHECKS:** To prevent hallucinations while preserving token limits, you must perform explicit visible checks in your thought process using these streamlined formats:
   ...
   - **For Clarity (All new Tasks/Events):** `CLARITY CHECK: [node] — Who: [...] What (Details/Scope): [...] When (Deadline/Date): [...] Why: [...]`. **STRICT DEFAULT:** `needs_clarification` MUST default to `true` for EVERY new Task or Event. You may ONLY set it to `false` if there is absolutely NO question that CAN be asked relating to the node DIRECTLY (i.e., every single one of the Who/What/When/Why fields is explicitly and comprehensively answered in the transcript). Do NOT excuse missing data. If even a minor detail is implied rather than stated, `needs_clarification` MUST be `true`.
    ...
    "mutations": [
      {
        ...
        "properties": {
          ...
          "needs_clarification": "[Boolean: STRICT DEFAULT is true. Only false if ZERO questions can be asked]",
